...
docs-check:
	node -e "const fs=require('fs');const path=require('path');const spec=path.join(process.cwd(),'.trellis','spec');const docs=[...walk(spec),path.join(process.cwd(),'README.md'),path.join(process.cwd(),'AGENTS.md'),path.join(process.cwd(),'CLAUDE.md')];const pattern=/\\[[^\\]]+\\]\\(([^)#]+)\\)/g;const missing=[];for(const doc of docs){const text=fs.readFileSync(doc,'utf8');for(const match of text.matchAll(pattern)){const rel=match[1];if(rel.includes('://')||rel.startsWith('#')) continue;const target=path.resolve(path.dirname(doc),rel);if(!fs.existsSync(target)) missing.push(`$${doc}: $${rel}`);}}if(missing.length>0){console.error(missing.join('\\n'));process.exit(1);}function walk(dir){if(!fs.existsSync(dir))return [];const entries=fs.readdirSync(dir,{withFileTypes:true});const files=[];for(const entry of entries){const fullPath=path.join(dir,entry.name);if(entry.isDirectory()){files.push(...walk(fullPath));continue;}if(entry.isFile()&&fullPath.endsWith('.md')){files.push(fullPath);}}return files;}"
